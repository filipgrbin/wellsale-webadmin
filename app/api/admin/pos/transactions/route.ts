...
/**
 * Same-origin proxy for GET /api/admin/pos/transactions.
 * Browser → webadmin (no CORS) → api.wellsale.cz with x-admin-key.
 */
export async function GET(request: NextRequest) {
  const adminKey = request.headers.get("x-admin-key") || ADMIN_KEY;
  const qs = request.nextUrl.searchParams.toString();
  const url = `${API_BASE}/api/admin/pos/transactions${qs ? `?${qs}` : ""}`;
